...
进程太重，每次切换的成本很高，有了线程的概念，每个进程下面可以有多个线程，任务由线程执行，线程的上下文切换成本远低于进程的上下文切换成本。

对于单CPU计算机而言，大部分操作系统多线程的执行是时间片轮转的抢占式调度，什么意思呢？就是将连续的时间分成一小段一小段的，每一段称为时间片，线程执行就是分配了一个时间片，执行时间到达后，调度其它线程执行（将后续的时间片分配给其它线程），由于CPU运转速度很快，在人能感知的时间范围内有大量的时间分片，每个时间片特别短，给人的感觉就是多个任务"同时"在执行。
...
首先了解下java的内存模型(JMM)，java有主内存和工作内存之分，每个线程在访问共享变量的时候都会将主内存数据copy到工作内存中，然后后面的读/写操作都在工作内存进行，一个线程修改了某个共享变量后什么时候同步到主内存是不确定的，这样对于其他线程可能看不到修改后的值，这就是内存可见性问题。

指令重排序:
现代的处理器为了提升运算速度，进行了很多优化，指令重排序就是其中一种。如果从单线程看某个程序执行，是按照代码顺序从上到下依次执行，但是实际上存在编译器指令重排序和运行期指令重排序，如果处理器发现在不影响执行结果的情况下，重新对指令排序能提升效率就会重排。这样对于单线程而言没有影响，但是对于别的线程看到的就不是顺序执行，这样会存在并发问题。

对，上面说的内存可见性和指令重排序通过volatile可以禁止掉。如果一个变量被volatile修饰，则每次读/写变量都会同步主内存的变量，从而保证了内存可见性。它可以通过在指令之间插入内存屏障(Memory Barrier)来禁止指令重排。
